...
public class DetectShape: MonoBehaviour
{
    ...
    [SerializeField] private float minAcum = 10f;
    ...
    [SerializeField] private float angleDb1;
    [SerializeField] private float angleDb2;
    [SerializeField] private float angleVariaton = 0;
    [SerializeField] private List<float> angles_drawing = new List<float>();
    [SerializeField] private List<float> angles_Variaton = new List<float>();
    ...
    void CalculateShapeVariation(List<float> angles_Variaton_Shape)
    {
        ...
        {
            case 1:
                GetCorrectShapeByAngles(size, Leap, angles_Variaton_Shape, 2.1f, angleTolerance);
                if (correctShape == 2.1f)
                    break;
                GetCorrectShapeByAngles(size, Fall, angles_Variaton_Shape, 2.2f, angleTolerance);
                break;

            case 2:
                GetCorrectShapeByAngles(size, Triangle, angles_Variaton_Shape, 3.1f, angleTolerance);
                if (correctShape == 3.1f)
                    break;
                GetCorrectShapeByAngles(size, Break, angles_Variaton_Shape, 3.2f, angleTolerance);
                break;

            case 3:
                GetCorrectShapeByAngles(size, Quadrat, angles_Variaton_Shape, 4.1f, angleTolerance +10);
                if (correctShape == 4.1f)
                    break;
                GetCorrectShapeByAngles(size, Rellotge, angles_Variaton_Shape, 4.2f, angleTolerance);
                break;

            case 4:
                GetCorrectShapeByAngles(size, Pentagon, angles_Variaton_Shape, 5.1f, angleTolerance);
                if (correctShape == 5.1f)
                    break;
                GetCorrectShapeByAngles(size, Eye, angles_Variaton_Shape, 5.2f, angleTolerance);
                if (correctShape == 5.2f)
                    break;
                GetCorrectShapeByAngles(size, Invis, angles_Variaton_Shape, 5.3f, angleTolerance);
                break;
        }
    }

    void GetCorrectShapeByAngles(int size, float[] shape, List<float> aVShape, float shapeId, float angleTolerance)
    {
        angleDiferencie = 0;
